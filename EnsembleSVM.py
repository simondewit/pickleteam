...
def main():
	...
	probabilityScores = predict_proba(test_tweets)
	print(probabilityScores[0])
	...
	ourOutput = open('EnsembleEngSVM.pickle','wt')
	for i in predicted_categories:
		ourOutput.write(i+"\n")


	# ourOutput = open('EnsembleEsSVM.txt','wt')
	# for i in predicted_categories:
	# 	ourOutput.write(i+"\n")
...
def classify(train_tweets, train_categories):
	...
	text_clf = Pipeline([#[('feats', FeatureUnion([
						 ...
						 #])),
						 ('classifier', SVC(probability=True, kernel="linear", random_state=42, max_iter=50, tol=None))])
